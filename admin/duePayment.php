           ...
           <!-- <div class="card mb-3">
            ...
            <div class="card-body">
              <div class="table-responsive">
                ...
                <?php                
                  $query = "SELECT * FROM clients WHERE status='active' && payment_status='unfinished' && due_status='due' && user_type='client'";
            ...
            ?>
                ...
                <?php  
                   ...
                   //       $sql5 = "SELECT * from `next_payment` WHERE `client_id`='".$res['client_id']."' order by `nextPayment_id` DESC LIMIT 1";
                   //          $result5 = mysqli_query($db,$sql5);
                   //          while($row5=mysqli_fetch_array($result5)){

                   //            echo "<td>" . date('F j, Y / g:i a',strtotime($row5['nextPayment_date'])) . "</td>";
  ...
  $payment=mysqli_query($db,"SELECT * from `payment` WHERE `client_id`='".$res['client_id']."' order by `payment_id` ASC LIMIT 1")or die(mysqli_error());
    while($pay=mysqli_fetch_array($payment)){
      ...
      if ($status == 'undue') {
        ...
        $sql = "SELECT * from `next_payment` WHERE `client_id`='".$res['client_id']."' order by `nextPayment_id` DESC LIMIT 1";
          $result = mysqli_query($db,$sql);
            while($row=mysqli_fetch_array($result)){

        echo "<td>" . date('F j, Y / g:i a',strtotime($row['nextPayment_date'])) . "</td>";
    ...
    while($pay=mysqli_fetch_array($payment)){
      ...
      if ($status == 'undue') {
        ...
        echo "<td>" . date('F j, Y / g:i a',strtotime($row['nextPayment_date'])) . "</td>";
          ...
          <div class="card mb-3">
            <div class="card-header">
              <i class="fas fa-money-bill-wave"></i> 1 Month
               Due Payment
               <span style="margin-left: 550px;">Payment Legend:
                 <span class="badge" style="font-size: 15px; background: #2ecc71"><small class="text-white">Finished</small></span>
                 <span class="badge" style="font-size: 15px; background: white"><small>Ongoing</small></span>
                 <span class="badge" style="font-size: 15px; background: red"><small class="text-white">Error</small></span>
               </span>
            </div>
            <div class="card-body">
              ...
              <?php
                  ...
                  $one=mysqli_query($db,"SELECT * from `payment` WHERE `client_id`='".$res['client_id']."' ORDER BY `payment_id` DESC LIMIT 1")or die(mysqli_error());
                  while($month=mysqli_fetch_array($one)){

                    $monthlydue = '2 month';
                    $monthlyduedate = new DateTime($month['date'] . $monthlydue);

                    $todayDate = new DateTime();


                    if ($todayDate >= $monthlyduedate) {

                      // echo "<span class=\"text-center\"; style=\"color: white; background: #2ecc71; margin-left: 415px;\";>No 1 Month Due Payments Yet!</span>";
    ...
    while($pay=mysqli_fetch_array($payment)){
      ...
      if ($status == 'undue') {
        ...
        echo "<td>" . date('F j, Y / g:i a',strtotime($row['nextPayment_date'])) . "</td>";
          ...
          <div class="card mb-3">
            <div class="card-header">
              <i class="fas fa-money-bill-wave"></i> 2 Month
               Due Payment
               <span style="margin-left: 550px;">Payment Legend:
                 <span class="badge" style="font-size: 15px; background: #2ecc71"><small class="text-white">Finished</small></span>
                 <span class="badge" style="font-size: 15px; background: white"><small>Ongoing</small></span>
