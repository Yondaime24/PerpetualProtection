<?php
    ...
    $query ="UPDATE `client_message` SET `status` = 'read' WHERE `message_id` = $message_id;";
    performQuery($query);

    if (isset($_POST['replyMessage_btn'])) {
    ...
    $query = "INSERT INTO admin_message (`client_id`, `requestReason`, `datetime`, `message`, `type`, `status`)VALUES('$client_id', 'N/A', CURRENT_TIMESTAMP, '$message', 'Message', 'unread')";
        ...
        echo "<script>window.open('clientMessage.php?message_id=$message_id','_self')</script>";
...
<html lang="en">
  ...
  <head>
    ...
    <title>Perpetual Protection | Client Message</title>
...
<div id="content-wrapper">
    ...
    <div class="container">
      <div class="card mx-auto">
        <div class="card-body">
         <?php
              $query=mysqli_query($db,"SELECT * FROM client_message WHERE message_id='$message_id'")or die(mysqli_error());
          ...
          <div class="form-row">
            <div class="col-md-6">
              <div class="form-label-group">
                <input id="fullname" type="text" class="form-control" value="<?php echo $row['full_name'];?>" readonly>
                <label id="label3"><b>From:</b></label>
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-label-group">
                <input id="fullname" type="text" class="form-control" value="<?php echo $row['contact_number'];?>" readonly>
          ...
          <div class="form-row">
            <div class="col-md-12">
              <div class="form-label-group">
                <input id="fullname" type="text" class="form-control" value="<?php echo date('F j, Y / g:i a',strtotime($row['datetime']))?>" readonly>
                <label id="label3"><b>Message Sent On:</b></label>
              </div>
            </div>
            </div>
          </div>
          ...
          ?>
             <a class="btn btn-outline-info" href="#" data-toggle="modal" data-target="#messageModal">Send Reply</a>
  ...
  <?php }?>
    ...
    <div class="container">
      ...
      <?php
        ...
        while($row=mysqli_fetch_array($query)){
      ?>
        <div class="card-header text-center"><span>To<br></span><b><?php echo $row['full_name'] ?></b></div>

        <div class="card-body">

        <form action="clientMessage.php?message_id=<?php echo $row['message_id'] ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="modal-footer">
          ...
          <input class="btn btn-primary btn-sm" type="submit" value="Send" name="replyMessage_btn">
        </div>
      </form>
   <?php }?>
    </div>
