...
<head>
      ...
      <title>Perpetual Protection | Edit Beneficiary 1</title>
...
<form class="" id="reg_form" action="beneficiary1Edit1.php" method="post" onsubmit="return checkInputs()" enctype="multipart/form-data">
    ...
    <?php echo display_error(); ?>
        ...
        <div class="user-details">
          ...
          <div class="input-box">
            ...
            ?>
               <input type="text" value="<?php echo $row['name'];?>"  name="bname1" id="bname1" autocomplete="off">
            <?php }?>
            ...
            <span class="details">Relation</span>
             <?php
                $query=mysqli_query($db,"SELECT relationship FROM beneficiary WHERE client_id='".$_SESSION['plan']['client_id']."' LIMIT 1")or die(mysqli_error());
                  while($row=mysqli_fetch_array($query)){
            ?>
                <input type="text" value="<?php echo $row['relationship'];?>" name="brelationship1" id="brelationship1" autocomplete="off">
            <?php }?>
            <i class="fas fa-check-circle"></i>
            <i class="fas fa-exclamation-circle"></i>
            <small>Error Message</small>
          ...
          <div class="input-box">
            ...
            <?php
                $query=mysqli_query($db,"SELECT contact_num FROM beneficiary WHERE client_id='".$_SESSION['plan']['client_id']."' LIMIT 1")or die(mysqli_error());
                  while($row=mysqli_fetch_array($query)){
            ?>
                <input type="text" value="<?php echo $row['contact_num'];?>" name="bcontact_num1" id="bcontact_num1" autocomplete="off">
            <?php }?>
            <i class="fas fa-check-circle"></i>
            <i class="fas fa-exclamation-circle"></i>
            <small>Error Message</small>
        ...
        <div class="button">
          <input type="submit" value="UPDATE" name="beneficiary1" id="submit">
        </div>
...
function displayImage(e) {
  ...
  <script src="../asset/js/1beneficiary2.js"></script>
