...
<head>
      ...
      <title>Perpetual Protection | Edit Beneficiary 2</title>
...
<form class="" id="reg_form" action="beneficiary2Edit1.php" method="post" onsubmit="return checkInputs()" enctype="multipart/form-data">
    ...
    <?php echo display_error(); ?>
        ...
        <div class="user-details">
          ...
          <div class="input-box" hidden="">
           ...
           <div class="input-box" hidden="">
            ...
            <?php
                $query=mysqli_query($db,"SELECT beneficiary_id FROM beneficiary WHERE client_id='".$_SESSION['plan']['client_id']."' LIMIT 1, 1")or die(mysqli_error());
          ...
          <div class="input-box">
            ...
            <?php
                $query=mysqli_query($db,"SELECT name FROM beneficiary WHERE client_id='".$_SESSION['plan']['client_id']."' LIMIT 1, 1")or die(mysqli_error());
                  while($row=mysqli_fetch_array($query)){
            ?>
               <input type="text" value="<?php echo $row['name'];?>"  name="bname2" id="bname2" autocomplete="off">
            <?php }?>
            <i class="fas fa-check-circle"></i>
            <i class="fas fa-exclamation-circle"></i>
            <small>Error Message</small>
            ...
            <span class="details">Relation</span>
             <?php
                $query=mysqli_query($db,"SELECT relationship FROM beneficiary WHERE client_id='".$_SESSION['plan']['client_id']."' LIMIT 1, 1")or die(mysqli_error());
                  while($row=mysqli_fetch_array($query)){
            ?>
                <input type="text" value="<?php echo $row['relationship'];?>" name="brelationship2" id="brelationship2" autocomplete="off">
            <?php }?>
            <i class="fas fa-check-circle"></i>
            <i class="fas fa-exclamation-circle"></i>
            <small>Error Message</small>
          ...
          <div class="input-box">
            ...
            <?php
                $query=mysqli_query($db,"SELECT contact_num FROM beneficiary WHERE client_id='".$_SESSION['plan']['client_id']."' LIMIT 1, 1")or die(mysqli_error());
                  while($row=mysqli_fetch_array($query)){
            ?>
                <input type="text" value="<?php echo $row['contact_num'];?>" name="bcontact_num2" id="bcontact_num2" autocomplete="off">
            <?php }?>
            <i class="fas fa-check-circle"></i>
            <i class="fas fa-exclamation-circle"></i>
            <small>Error Message</small>
        ...
        <div class="button">
          <input type="submit" value="UPDATE" name="beneficiary2" id="submit">
        </div>
...
function displayImage(e) {
  ...
  <script src="../asset/js/2beneficiary2.js"></script>
