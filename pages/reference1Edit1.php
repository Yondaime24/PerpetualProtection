...
<head>
     ...
     <link rel="apple-touch-icon" sizes="76x76" href="../images/insurance_icon.png">
      ...
      <title>Perpetual Protection | Edit Reference 1</title>
...
<form class="" id="reg_form" action="reference1Edit1.php" method="post" onsubmit="return checkInputs()" enctype="multipart/form-data">
    ...
    <?php echo display_error(); ?>
        ...
        <div class="user-details">
          ...
          <div class="input-box">
            ...
            ?>
               <input type="text" value="<?php echo $row['name'];?>"  name="name1" id="name1" autocomplete="off">
            <?php }?>
          ...
          <div class="input-box">
           ...
           <?php
                $query=mysqli_query($db,"SELECT address FROM clients_references WHERE client_id='".$_SESSION['plan']['client_id']."' LIMIT 1")or die(mysqli_error());
                  while($row=mysqli_fetch_array($query)){
            ?>
               <input type="text" value="<?php echo $row['address'];?>"  name="address1" id="address1" autocomplete="off">
            <?php }?>
            <i class="fas fa-check-circle"></i>
            <i class="fas fa-exclamation-circle"></i>
            <small>Error Message</small>
            ...
            <span class="details">Contact Number</span>
             <?php
                $query=mysqli_query($db,"SELECT contact_num FROM clients_references WHERE client_id='".$_SESSION['plan']['client_id']."' LIMIT 1")or die(mysqli_error());
                  while($row=mysqli_fetch_array($query)){
            ?>
                <input type="text" value="<?php echo $row['contact_num'];?>" name="contact_num1" id="contact_num1" autocomplete="off">
            <?php }?>
            <i class="fas fa-check-circle"></i>
            <i class="fas fa-exclamation-circle"></i>
            <small>Error Message</small>
          ...
          <div class="input-box">
            ...
            <?php
                $query=mysqli_query($db,"SELECT relation FROM clients_references WHERE client_id='".$_SESSION['plan']['client_id']."' LIMIT 1")or die(mysqli_error());
                  while($row=mysqli_fetch_array($query)){
            ?>
                <input type="text" value="<?php echo $row['relation'];?>" name="relation1" id="relation1" autocomplete="off">
            <?php }?>
            <i class="fas fa-check-circle"></i>
            <i class="fas fa-exclamation-circle"></i>
            <small>Error Message</small>
        ...
        <div class="button">
          <input type="submit" value="UPDATE" name="reference1" id="submit">
        </div>
...
function displayImage(e) {
  ...
  <script src="../asset/js/1reference1.js"></script>
